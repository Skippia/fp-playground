 ...
 */
const genA = (max: number) => A.unfold(0, n => (n < max ? O.some([n, n + 1]) : O.none))

function generateWorld(sizeX: number, sizeY: number, random: boolean = false): World {
  const xs = genA(sizeX)
  const ys = genA(sizeY)

  const genCell = () => (random ? pipe(R.randomBool(), r => (r ? alive : dead)) : dead)
...
const getCellAt
  = (world: World) =>
    ({ x, y }: Coordinates): O.Option<Cell> =>
      pipe(world, A.lookup(x), O.chain(A.lookup(y)))
...
const world = generateWorld(cellsX, cellsY, random)
